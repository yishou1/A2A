...
class NacosRegistry:
    ...
    MAX_METADATA_JSON_LENGTH = 1000
    SCHEDULING_METADATA_KEYS = (
        "role",
        "status",
        "skill_ids",
        "agent_run_state",
    ...
    @staticmethod
    def _nacos_metadata(metadata):
        """Convert runtime metrics to Nacos' Map[str, str] wire contract."""
        normalized = {}
        ...
        return NacosRegistry._compact_metadata(normalized)

    @classmethod
    def _compact_metadata(cls, metadata):
        """Fit metadata under Nacos' strict 1024-character instance limit."""
        def serialized_length(value):
            # nacos-sdk-python serializes metadata with ensure_ascii=True.
            return len(json.dumps(value, ensure_ascii=True))

        if serialized_length(metadata) <= cls.MAX_METADATA_JSON_LENGTH:
            return metadata

        compact = {}
        for key in cls.SCHEDULING_METADATA_KEYS:
            if key in metadata:
                compact[key] = metadata[key]
        for key, value in metadata.items():
            if key.startswith(("lease_", "unavailable_")):
                compact[key] = value

        # skill_ids is the dispatch contract. The verbose localized `skills`
        # description is optional and is the largest field on TIA agents.
        for key, value in metadata.items():
            if key in compact or key == "skills":
                continue
            candidate = {**compact, key: value}
            if serialized_length(candidate) <= cls.MAX_METADATA_JSON_LENGTH:
                compact[key] = value

        if serialized_length(compact) > cls.MAX_METADATA_JSON_LENGTH:
            raise ValueError("Required Nacos scheduling metadata exceeds the safe limit")
        return compact
    ...
    def register_service(
        ...
        metadata = self._nacos_metadata(metadata)
