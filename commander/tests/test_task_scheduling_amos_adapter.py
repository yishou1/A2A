...
class TestAmosAdapter(unittest.TestCase):
    ...
    def test_explicit_platform_inventory_does_not_invent_strike_assets(self):
        payload = {
            "tasks": [{"target_id": "E1", "threat_score": 0.8}],
            "platforms": [
                {
                    "platform_id": "AEW-1",
                    "role": "sensor",
        ...
        situation = situation_from_amos(payload)
        schedule = mock_schedule_from_situation(situation)

        self.assertEqual(situation.strike_assets, [])
        self.assertEqual(schedule["reattack_plan"], [])
