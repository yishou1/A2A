...
class FakeCommanderClient:
    ...
    def get_checkpoint(self, workflow_id):
        ...
        if workflow_id not in self.checkpoints:
            raise UpstreamError("COMMANDER_NOT_FOUND", "checkpoint not found", 404, False)
        return copy.deepcopy(self.checkpoints[workflow_id])
...
class GatewayTestCase(unittest.TestCase):
    ...
    def test_verified_snapshot_becomes_stable_commander_mission_context(self):
        snapshot = make_snapshot()
        snapshot["tracks"] = [{
        ...
        snapshot["simulation_chains"][0]["submission_context"] = {
        ...
        service = GatewayService(
            self.config,
            store=self.store,
            amos_client=FakeAmosClient(snapshot=snapshot),
            commander_client=self.commander,
        )

        service.submit(WorkflowSubmitV1.model_validate(submit_payload()))

        mission = self.commander.submit_calls[0]["initial_context"]["mission_input"]
        self.assertEqual(mission["contacts"][0]["contact_id"], "TRK-AMOS-01")
        self.assertEqual(mission["contacts"][0]["geo"], {"lat": 22.123456, "lon": 121.654321})
        self.assertEqual(mission["contacts"][0]["classification"], "fast_attack_craft")
        self.assertEqual(
            mission["contacts"][0]["metadata"]["prior_classification"],
            "fast_attack_craft",
        )
        self.assertGreater(mission["contacts"][0]["metadata"]["speed_kts"], 0)
        self.assertTrue(mission["contacts"][0]["metadata"]["ais_observed"])
        self.assertEqual(
            mission["perception_frames"][0]["detections"][0]["metadata"]["amos_track_id"],
            "TRK-AMOS-01",
        )
        self.assertGreater(mission["perception_frames"][0]["detections"][0]["speed"], 0)
        self.assertEqual(mission["metadata"]["director_checkpoint_id"], "MAR-CP-PERCEPTION")
    ...
    def test_completed_projection_resolves_checkpoint_outputs(self):
        projection = self.service.submit(WorkflowSubmitV1.model_validate(submit_payload()))
        ...
        self.commander.checkpoints[workflow_id] = {
            ...
            "context": {
                ...
                "workflow_result": {
                    "status": "completed",
                    "outputs": {"tracking_result": {"$ref": "context.tracking_result"}},
                    "activity_results": [{
                        "activity_id": "activity-track",
        ...
        current = self.service.get_projection(workflow_id)
