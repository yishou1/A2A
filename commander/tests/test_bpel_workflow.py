...
class BPELWorkflowTest(unittest.TestCase):
    ...
    def test_phase_workflows_only_dispatch_their_ooda_stage(self):
        expected = {
            "observe_workflow.bpel": ["tactical_intelligence", "track_threat"],
            "orient_workflow.bpel": ["track_threat", "track_threat"],
            "decide_workflow.bpel": [
        ...
        for filename, roles in expected.items():
            definition = BPELWorkflowCatalog(PROJECT_ROOT).load(
                f"integrated_system/workflows/{filename}"
            )
            actual = [
                item["role"] for item in definition.initial_work_list(f"wf-{filename}")
                if item["type"] == "invoke"
            ]
    ...
    def test_decide_phase_builds_valid_planning_and_compliance_contracts(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            commander = CommanderAgent(
                ...
                workflow_id="wf-decide-contract",
                state_dir=temp_dir,
            )
            context = commander.initial_workflow_context()
                ...
                "risk_assessments": [
                    {
                        ...
                        "threat_score": 84.0,
                        "probability": 0.84,
                        "rationale": "Confirmed hostile contact.",
                    }
                ],
                "scheduled_tasks": [
            ...
            context["task_scheduling_result"] = [
                commander._make_context_entry(context["planning_input"])
            ]
            planning_activity = next(
                item
                for item in commander.bpel_definition._activatities
                if item.role == "decision_planning"
            )
            planning_payload, _ = commander._build_bpel_task_payload(
                planning_activity, context
            )
            planning_request = AgentRequest.model_validate(
                build_agent_request_payload(
                    "decision_planning_agent", planning_payload
                )
            )
            self.assertEqual(planning_request.request_id, "wf-decide-contract")
            self.assertEqual(planning_request.scheduled_tasks[0].id, "TASK-1")
            ...
            compliance_activity = next(
                item
                for item in commander.bpel_definition._activatities
                if item.role == "compliance_authorization"
            )
            compliance_payload, _ = commander._build_bpel_task_payload(
                compliance_activity, context
            )
            compliance_request = AgentRequest.model_validate(
                build_agent_request_payload(
                    "compliance_authorization_agent", compliance_payload
                )
            )
    ...
    def test_act_phase_requires_operator_authorization_evidence(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            commander = CommanderAgent(
                ...
                workflow="bpel",
                workflow_file="integrated_system/workflows/act_workflow.bpel",
                workflow_id="wf-act-authorized",
                state_dir=temp_dir,
                initial_context={
                    "mission_input": {
                        "contacts": [{"track_id": "TRK-1", "contact_id": "TRK-1"}],
                        "friendly_platforms": [{"platform_id": "ESCORT-01"}],
                        "stage_transfer": {
                            "phase": "ASSESS",
                            "supplemental_inputs": {
            ...
            context = commander.initial_workflow_context()
            context.update(commander.initial_context)
            payload, _ = commander.build_task_payload(
                "simulation_execution", context, activatity_index=1
            )

        compliance = payload["input"]["results"]["compliance_authorization"]["output_data"]
        self.assertEqual(compliance["decision"], "approved")
        self.assertTrue(compliance["approved_for_demo_handoff"])
        self.assertEqual(
            compliance["authorization_evidence"]["weapon_id"], "WPN-1"
        )
