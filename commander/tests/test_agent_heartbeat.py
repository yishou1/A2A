...
class AgentHeartbeatTest(unittest.TestCase):
    ...
    def test_register_service_normalizes_metadata_for_nacos(self):
        registry = NacosRegistry(server_addresses="127.0.0.1:8848")
        captured = {}
        ...
        registry.client.add_naming_instance = fake_add_naming_instance
        registry._start_heartbeat = lambda **kwargs: object()
        ...
        self.assertEqual(captured["cpu"], "88.5")
        self.assertEqual(captured["ready"], "true")
        self.assertEqual(captured["models"], '["one","two"]')
        self.assertNotIn("unavailable", captured)
        self.assertTrue(all(isinstance(value, str) for value in captured.values()))
