...
from decision_support.capabilities.formulas import logistic_probability
...
from decision_support.knowledge.rule_tables import (
...
class ComplianceCapabilities(Protocol):
    """Model capability consumed by the compliance workflow."""
...
class FormulaComplianceCapabilities:
    ...
    def calibrate(
        ...
        return _calibrate_compliance_result(result, request), {
            "algorithm_id": "compliance_risk_formula",
...
def _medium_compliance_authorization(request: AgentRequest) -> dict:
    return execute_medium_compliance_authorization(
        request, FormulaComplianceCapabilities()
    )


def execute_medium_compliance_authorization(
    request: AgentRequest,
    capabilities: ComplianceCapabilities,
) -> dict[str, Any]:
    """Compose deterministic checks, evidence retrieval, and risk calibration."""
    result = evaluate_compliance(request, use_rule_table=True)
    payload = {
        **result.model_dump(mode="json"),
        "method": "rule_table_rag_logistic_calibration",
        "algorithm_stages": ["structured_rule_evidence_retrieval"],
        "rule_table_version": "law-of-war-demo-v1",
    }
    calibration, runtime = capabilities.calibrate(result, request)
    payload.update(calibration)
    payload.update(_compliance_rag_payload(result, request))
    payload["algorithm_stages"].append(runtime["algorithm_id"])
    payload["model_runtime"] = {"compliance_risk": runtime}
    return payload
...
def _logistic_probability(features: dict[str, float], weights: dict[str, float]) -> float:
    return logistic_probability(features, weights)
