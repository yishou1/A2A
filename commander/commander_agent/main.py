...
class CommanderAgent:
    def __init__(
        ...
        self.agent_discovery_retries = max(
            0,
            int(os.environ.get("A2A_AGENT_DISCOVERY_RETRIES", "4")),
        )
        self.agent_discovery_retry_interval = max(
            0.0,
            float(os.environ.get("A2A_AGENT_DISCOVERY_RETRY_INTERVAL", "0.25")),
        )
    ...
    def _delegate_task_with_lease(self, role_needed: str, task_payload: dict, stream: bool = False):
        ...
        while True:
            ...
            if lease is None:
                if not attempted_keys and discovery_retries < self.agent_discovery_retries:
                    discovery_retries += 1
                    self._trace(
                        "agent_discovery_retry",
                        role=role_needed,
                        work_item=work_item,
                        retry=discovery_retries,
                        max_retries=self.agent_discovery_retries,
                        retry_interval_seconds=self.agent_discovery_retry_interval,
                    )
                    print(
                        f"[WAIT] No idle {role_needed} agent discovered; "
                        f"retrying {discovery_retries}/{self.agent_discovery_retries}."
                    )
                    time.sleep(self.agent_discovery_retry_interval)
                    continue
    ...
    @staticmethod
    def _normalize_authorization(value):
        authorization = dict(value or {}) if isinstance(value, dict) else {}
        if authorization:
    ...
    def _normalize_context(self, context: dict):
        ...
        normalized["authorization"] = self._normalize_authorization(
    ...
    def build_task_payload(self, role: str, context: dict, activatity_index: int = None, **legacy_kwargs):
        ...
        if role == "simulation_execution":
            ...
            supplemental = (
                stage_transfer.get("supplemental_inputs")
                if isinstance(stage_transfer.get("supplemental_inputs"), dict)
                else {}
            )
            operator_authorization = (
                supplemental.get("operator_authorization")
                if isinstance(supplemental.get("operator_authorization"), dict)
                else {}
            )
    ...
    def _decision_agent_request(self, context: dict) -> dict:
        ...
        request["authorization"] = deepcopy(
            self._normalize_authorization(
                request.get("authorization", context.get("authorization"))
            )
        )
        request["request_id"] = self.workflow_id
        return request
    ...
    def _build_bpel_task_payload(self, activatity: BPELActivatity, context: dict):
        with self._checkpoint_lock:
            ...
            if dispatch_key == "decision_planning":
                input_payload["agent_request"] = self._decision_agent_request(context)
            elif dispatch_key == "compliance_authorization":
                planning_input = self._decision_agent_request(context)
                planning_result = self._latest_context_value(
                    context, "decision_planning_result"
                ) or {}
                if isinstance(planning_result, dict):
                    planning_input["candidate_plans"] = deepcopy(
                        planning_result.get(
                            "candidate_plans", context.get("candidate_plans", [])
                        )
                    )
                planning_input["authorization"] = deepcopy(
                    self._normalize_authorization(context.get("authorization"))
                )
