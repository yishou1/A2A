...
def situation_from_amos(payload: dict[str, Any]) -> BattlefieldSchedulingState:
    ...
    # Legacy standalone requests omitted the platform field entirely and
    # relied on demo assets. An explicit AMOS platform inventory is
    # authoritative: do not invent a weapon resource when it contains none.
    if not strike_assets and "platforms" not in data:
        strike_assets = [
