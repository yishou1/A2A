...
from app.nacos_register import NacosRegistrar, NacosSettings, _wire_metadata
...
def test_nacos_wire_metadata_fits_limit_and_keeps_dispatch_contract(monkeypatch):
    monkeypatch.setenv("SERVICE_IP", "127.0.0.1")
    settings = NacosSettings.from_env()

    metadata = _wire_metadata(settings.metadata)

    assert metadata["role"] == "track_threat"
    assert metadata["status"] == "idle"
    assert "threat_ranking" in metadata["skill_ids"]
    assert metadata["active_tasks"] == "0"
    assert len(json.dumps(metadata, ensure_ascii=True)) <= 1000
