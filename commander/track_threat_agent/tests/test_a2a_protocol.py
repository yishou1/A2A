...
from app.main import (
    _perception_from_a2a_task,
    _requested_skills,
    send_message,
    verify_a2a_token,
    well_known_a2a_agent_card,
)
...
def test_phase_workflow_unwraps_mission_perception_frames():
    payload = _perception_from_a2a_task({
        "workflow_id": "wf-orient",
        "work_item": "wf-orient:tracking",
        "input": {
            "mission_input": {
                "scene": {"scenario_id": "maritime-convoy-air-defense"},
                "contacts": [
                    {
                        ...
                        "detections": [
                            {
                                ...
                                "lon": 121.2,
                                "confidence": 0.9,
                                "metadata": {"amos_track_id": "TRK-FISHING"},
                            }
                        ]
                    }
                ],
    ...
    assert payload.scene["scenario_id"] == "maritime-convoy-air-defense"
    assert [item.detection_id for item in payload.detections] == ["TRK-FISHING"]
    assert payload.detections[0].metadata["source_class"] == "fishing_vessel"
    assert payload.detections[0].metadata["label"] == "neutral"
...
async def test_send_message_accepts_a2a_task_payload():
    ...
    assert (
        body["artifact"]["summary"]["execution"]["mode"]
        == "llm_planned_algorithm_library_with_local_fallback"
    )
...
@pytest.mark.anyio
async def test_tracking_state_is_reset_when_a2a_workflow_changes():
    main.reset_runtime_state()
    intelligence = {
        ...
        "targets": [
            {
                ...
                "class": "ship",
                "geo": {"lat": 22.1, "lon": 121.5, "alt_m": 0.0},
                "confidence": 0.98,
            },
            {
                "track_id": "TRK-AMOS-02",
                "class": "ship",
                "geo": {"lat": 22.2, "lon": 121.6, "alt_m": 0.0},
                "confidence": 0.97,
            },
        ],
    }

    for workflow_id in ("wf-first", "wf-second"):
        task = _commander_task(
            work_item=f"{workflow_id}:tracking",
            required_skill="trajectory_tracking",
            output_hint="tracking_result",
            input_payload={"cognition_result": [{"value": intelligence}]},
        )
        task["workflow_id"] = workflow_id
        body = await send_message(task, token="unit-test")
        tracks = body["output"]["tracking_result"]["tracks"]
