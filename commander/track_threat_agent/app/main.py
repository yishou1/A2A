...
def _ensure_active_workflow(workflow_id: str) -> None:
    global _active_workflow_id
    normalized = str(workflow_id or "").strip()
    if normalized and normalized != _active_workflow_id:
        tracker.reset()
        reset_adapter_cache()
        _active_workflow_id = normalized
...
def _process_payload(
    ...
    *,
    ...
    _ensure_active_workflow(workflow_id)
...
def _process_a2a_task(
    ...
    _ensure_active_workflow(str(task_payload.get("workflow_id") or ""))
    request_id = str(
...
def _perception_from_a2a_task(task_payload: Dict[str, Any]) -> PerceptionResultRequest:
    ...
    if "detections" not in input_payload and isinstance(mission_input, dict):
        frames = [
            frame
            for frame in mission_input.get("perception_frames") or []
            if isinstance(frame, dict)
        ]
        contacts = {
            str(contact.get("track_id") or contact.get("contact_id")): contact
            for contact in mission_input.get("contacts") or []
            if isinstance(contact, dict)
            and (contact.get("track_id") or contact.get("contact_id"))
        }
        detections: list[dict[str, Any]] = []
        for frame in frames[-10:]:
            for raw in frame.get("detections") or []:
                if not isinstance(raw, dict):
                    continue
                detection = deepcopy(raw)
                metadata = (
                    deepcopy(detection.get("metadata"))
                    if isinstance(detection.get("metadata"), dict)
                    else {}
                )
                stable_track_id = str(
                ...
                contact = contacts.get(stable_track_id, {})
                contact_metadata = (
                    contact.get("metadata")
                    if isinstance(contact.get("metadata"), dict)
                    else {}
                )
                classification = str(
                    contact.get("classification")
                    or contact_metadata.get("prior_classification")
                    or "unknown"
                ).lower()
                if stable_track_id:
                    detection["detection_id"] = stable_track_id
        ...
        input_payload = {
            "algorithm_level": mission_input.get("algorithm_level", "medium"),
