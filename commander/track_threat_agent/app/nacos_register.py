...
_NACOS_METADATA_LIMIT = 1000
_NACOS_REQUIRED_KEYS = (
    "agent_id",
    "role",
    "status",
    "skill_ids",
...
def _wire_metadata(metadata: Dict[str, str]) -> Dict[str, str]:
    """Preserve dispatch fields while respecting Nacos' 1024-char limit."""
    normalized = {str(key): str(value) for key, value in metadata.items() if value is not None}

    def size(value: Dict[str, str]) -> int:
        return len(json.dumps(value, ensure_ascii=True))

    if size(normalized) <= _NACOS_METADATA_LIMIT:
        return normalized

    compact = {key: normalized[key] for key in _NACOS_REQUIRED_KEYS if key in normalized}
    for key, value in normalized.items():
        if key.startswith((_LEASE_METADATA_PREFIXES + _CIRCUIT_METADATA_PREFIXES + _UNAVAILABLE_METADATA_PREFIXES)):
            compact[key] = value
    for key, value in normalized.items():
        if key in compact or key == "skills":
            continue
        candidate = {**compact, key: value}
        if size(candidate) <= _NACOS_METADATA_LIMIT:
            compact[key] = value
    if size(compact) > _NACOS_METADATA_LIMIT:
        raise ValueError("Required Track-Threat Nacos metadata exceeds the safe limit")
    return compact
...
class NacosSettings:
    ...
    def from_env(cls) -> "NacosSettings":
        ...
        metadata = {
            ...
            "algorithm_library_allowed_ids": "multimodal_feature_fuser,trajectory_predictor,graph_relation_reasoner,threat_priority_random_forest",
...
class NacosRegistrar:
    ...
    def _update_instance_metadata_http(self) -> None:
        ...
        params = {
            ...
            "clusterName": "DEFAULT",
            "groupName": self.settings.group_name,
            "metadata": json.dumps(wire_metadata, separators=(",", ":")),
            "ephemeral": "true",
            "weight": "1.0",
            "enabled": "true",
            "healthy": "true",
    ...
    def _instance_http_request(self, method: str, *, include_metadata: bool) -> None:
        ...
        if include_metadata:
            params["metadata"] = json.dumps(
                _wire_metadata(self.settings.metadata), separators=(",", ":")
            )
        if self.settings.namespace and self.settings.namespace != "public":
            params["namespaceId"] = self.settings.namespace
