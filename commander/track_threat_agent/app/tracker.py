...
class MultiTargetTracker:
    ...
    def _new_track(self, detection: Detection) -> TrackState:
        ...
        source_track_id = str(detection.metadata.get("source_track_id") or "").strip()
        # The legacy TacticalIntelligence targets contract exposes a stable
        # track_id. Preserve it for Commander compatibility, while ordinary
        # raw detections continue to receive an Agent-generated ID.
        track_id = (
            source_track_id
            if source_track_id and source_track_id not in self.tracks
            else f"trk-{uuid4().hex[:10]}"
        )
        track = TrackState(
            track_id=track_id,
