...
def _contact_targets(mission_input: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for index, contact in enumerate(mission_input.get("contacts", []), start=1):
        ...
        lat, lon = geo.get("lat"), geo.get("lon", geo.get("lng"))
        if lat is None or lon is None:
            try:
                lat_text, lon_text = location.split(",", 1)
                lat, lon = float(lat_text), float(lon_text)
            except (TypeError, ValueError):
                lat, lon = 30.5 + (index * 0.012), 114.3 + (index * 0.015)
        threat_score = contact.get("threat_level", 0.5)
        try:
            threat_score = float(threat_score)
        except (TypeError, ValueError):
            threat_score = 0.5
        track_id = str(contact.get("track_id") or contact.get("contact_id") or f"T-{index:04d}")
        targets.append(
            {
                "track_id": track_id,
                "class": object_class,
                "label": contact.get("classification") or ("hostile" if threat_score >= 0.6 else "unknown"),
                "affiliation": contact.get("affiliation") or ("red" if threat_score >= 0.6 else "unknown"),
                "threat_level": "high" if threat_score >= 0.75 else "medium",
                "geo": {"lat": float(lat), "lon": float(lon), "alt_m": 120.0 if object_class != "ship" else 0.0},
                "confidence": float(contact.get("confidence") or metadata.get("confidence") or round(min(0.99, 0.55 + threat_score * 0.4), 4)),
