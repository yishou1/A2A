...
SKILL_ALGORITHM_DEFAULTS: dict[str, tuple[str, ...]] = {
    # CMS is the authority for multi-source association and filtering.  This
    # Agent synchronizes its stable track identities, but does not ask the
    # algorithm library to create a second set of tracks.
    "trajectory_tracking": (),
...
SKILL_ALGORITHM_ALLOWLIST: dict[str, frozenset[str]] = {
    ...
    "group_threat_ranking": frozenset({"multimodal_feature_fuser", "graph_relation_reasoner", "threat_priority_random_forest"}),
        ...
        if self.settings.llm_enabled:
            try:
                ...
                llm_plan = self.llm.plan(
                    ...
                    algorithms=[self._catalog_view(item) for item in self._active_by_id.values()],
