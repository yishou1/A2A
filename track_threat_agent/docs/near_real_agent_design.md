...
当前阶段开始从 Demo 算法过渡到计划书算法契约。默认 provider 为：
...
已有本地实现不再作为主算法口径，而是作为 fallback：

```text
baseline_motion_provider
baseline_dbn_runtime
metadata_kg_transformer_adapter
xai_evidence_runtime
```

当前算法已经消费 TacticalIntelligenceAgent 的上游语义字段：`threat_level`、`affiliation`、`label`、`source_class` 和 `knowledge_graph.nodes[].relations` 会进入 DBN 威胁评估、资产影响分析、编组凝聚度和 KG+Transformer 语义 adapter。训练版 ST-GNN 或师兄公共算法库完成后，可替换 provider 内部实现。
...
- 训练版 ST-GNN 权重尚未随仓库提供，当前由 baseline motion provider 保证联调可运行；
...
> 当前版本不是生产 Agent，但已经按近真实 Agent 方式封装。它支持 A2A Agent Card、sendMessage、sendMessageStream、workflow_id、work_item 幂等、work_list 查询、Nacos role/status metadata、health 运行态和计划书算法 provider。算法主线已切换为 ST-GNN、DBN、KG+Transformer、XAI 契约；当前 baseline 只用于保证联调可运行，后续公共算法库完成后只需要替换 provider，不需要改变对外协议。
