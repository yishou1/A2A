...
- `dynamic_bayesian_network`：威胁状态后验概率评估；
- `kg_transformer_semantic_sitrep`：知识图谱 + Transformer 语义态势契约；
...
kg_transformer_self_attention_runtime
dbn_with_coa_probability_runtime
xai_evidence_runtime
```

实际流程是：`medium` 档先用协方差 Kalman 更新 track 状态，再用 IMM 生成三类运动假设；ST-GNN 本地运行时把目标构成动态图，通过两层消息传递输出节点 embedding、边注意力和速度/加速度修正；KG+Transformer 模块把 TacticalIntelligenceAgent 的 `threat_level`、`affiliation`、`label`、`source_class` 和 `knowledge_graph.nodes[].relations` 转成语义 token，通过 self-attention 输出语义态势因子和意图概率；DBN 模块在 low/medium/high 威胁状态上做时序后验更新，并输出 COA 概率。
...
skills=trajectory_tracking,st_gnn_dynamic_entity_tracking,dynamic_bayesian_network_threat_assessment,kg_transformer_semantic_sitrep,group_detection,protected_asset_impact_analysis,xai_evidence_generation
```
...
> 当前版本不是生产 Agent，但已经按近真实 Agent 方式封装。它支持 A2A Agent Card、sendMessage、sendMessageStream、workflow_id、work_item 幂等、work_list 查询、Nacos role/status metadata、health 运行态和计划书算法 provider。算法层已经在本地实现协方差 Kalman、IMM、ST-GNN 消息传递、KG+Transformer 自注意力语义推理、DBN+COA 后验评估和 XAI 证据链；后续公共算法库完成后主要替换 provider 或模型权重，不需要改变对外协议。
