...
def main():
    ...
    save_dir = 'outputs/demo_llama'  # Define save directory for SynapseRAG objects (each LLM/Embedding model combination will create a new subdirectory)
    ...
    log_path = setup_logging(
        ...
        run_name=f"demo-local-{datetime.now().strftime('%Y%m%d-%H%M%S')}",
        level=os.getenv("LOG_LEVEL", "INFO")
    )
    logger = logging.getLogger(__name__)
    logger.info("Writing demo (local) logs to %s", log_path)
    ...
    logger.info("Demo (local) QA results: %s", results)
