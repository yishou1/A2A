    ...
    def _load_templates(self) -> None:
        ...
        for filename in os.listdir(self.templates_dir):
            if filename.endswith(".py") and filename != "__init__.py":
                ...
                try:
                    try:
                        module_name = f"src.synapserag.prompts.templates.{script_name}"
                        module = importlib.import_module(module_name)
                    except ModuleNotFoundError:
                        module_name = f".prompts.templates.{script_name}"
                        module = importlib.import_module(module_name, 'synapserag')
    ...
    def print_template(self, name: str) -> None:
        ...
        try:
            ...
            if isinstance(template, Template):
                ...
                for item in template:
                    logger.info("Role: %s, Content: %s", item["role"], item["content"])
