...
@dataclass
class LLMResponse:
    """Normalized LLM response with tuple compatibility for legacy callers."""
    ...
    def __getitem__(self, index):
        return (self.text, self.metadata, self.cache_hit)[index]


def coerce_llm_response(value: Any) -> LLMResponse:
    if isinstance(value, LLMResponse):
        return value
    if isinstance(value, tuple):
        if len(value) == 3:
            return LLMResponse(text=value[0], metadata=value[1] or {}, cache_hit=bool(value[2]))
        if len(value) == 2:
            return LLMResponse(text=value[0], metadata=value[1] or {}, cache_hit=False)
