...
def softmax(x, T: float = 1.0):
    """Temperature softmax used by agentic PPR resets."""
...
def masked_softmax(x, mask, T: float = 1.0):
    """Softmax over masked entries; falls back to normal softmax when mask empty."""
    x = np.asarray(x, dtype=float)
    ...
    y = np.full_like(x, -np.inf, dtype=float)
    y[m] = x[m]
    T = max(float(T), 1e-6)
    z = (y - np.max(y[m])) / T
    e = np.exp(z)
