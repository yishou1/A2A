...
## Decision-support capability packages

The two core services are orchestration algorithms. Their model capabilities
are independently versioned packages:

| algorithm package | Purpose | Input |
|---|---|---|
| `decision_plan_recommender_onnx:1.0.0` | Candidate-plan recommendation probability | `features` shape `[1, 8]` |
| `target_trend_predictor_onnx:1.0.0` | Target trend score | `sequence` shape `[1, 12, 4]` |
| `compliance_risk_scorer_onnx:1.0.0` | Compliance risk probability | `features` shape `[1, 6]` |

Each package owns its model, metadata, tensor contract, preprocessing,
postprocessing, schemas, and evaluation records under `examples/<algorithm_id>/1.0.0`.
The trained logistic models are regenerated by
`scripts/train_logistic_onnx_models.py`; the LSTM package is generated by
`scripts/export_target_trend_lstm.py`.

At runtime, `decision_capabilities.py` adapts shared business features to these
generic tensor contracts. `decision_planning_core` composes recommendation and
trend prediction; `compliance_authorization_core` composes compliance-risk
calibration. Missing models, short trend histories, and unavailable
`onnxruntime` use deterministic formula fallbacks and report the reason in
`model_runtime`.
...
- The cores use the shared RAG interface; `RAG_BACKEND=synapserag` keeps retrieval behind the SynapseRAG HTTP boundary.
- Services do not depend on BPEL, Nacos, prompts, LLM selection, or the A2A Agent runtime.
