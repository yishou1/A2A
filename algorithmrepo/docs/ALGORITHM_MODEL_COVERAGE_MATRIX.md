...
| M05 | 随机森林模型 | `threat_priority_random_forest` | `COMPLETE` | 已训练并固化 sklearn Random Forest；参考数据集、训练脚本、模型 SHA256、留出集指标、HTTP 与 `algolib` 严格验收均已闭环 | 使用真实业务标注数据重新训练并独立评估后再用于业务决策 |
| M06 | 传统神经网络模型 | `supcon_meta_classifier` | `COMPLETE` | 已训练并固化 small profile 的两层投影 MLP 与类别原型（99,200 参数）；1600 条确定性合成嵌入及 400 条分层留出集、训练脚本和 safetensors 权重均记录 SHA256；留出集 accuracy/macro-F1 均为 1.0；缺失权重不再随机初始化，HTTP 与 `algolib` 严格真实验收通过 | 使用真实脱敏融合嵌入重训和独立评估；当前合成聚类指标不得解释为实际目标识别精度 |
| M07 | 朴素贝叶斯网络 | `intent_gaussian_naive_bayes` | `COMPLETE` | 已训练并固化 GaussianNB；合成参考数据、后验概率、模型 SHA256、留出集指标、HTTP 与 `algolib` 严格验收均已闭环 | 使用真实观测数据重训并完成概率校准、漂移监测与独立评估 |
| M08 | 生成对抗网络 | `conditional_tabular_gan` | `COMPLETE` | 已完成 PyTorch 条件 GAN 对抗训练、生成器固化、条件生成、源码/数据/模型 SHA256、分布指标、HTTP 与 `algolib` 严格验收 | 使用真实脱敏数据重训；合成输出不得冒充真实观测 |
...
| M13 | 强化学习模型 | `marl_ppo_task_scheduler`；补充候选 `marl_dynamic_router` | `COMPLETE` | 已修复智能体动作索引和同类资源不可区分问题，训练并固化参数共享 MARL-PPO Actor-Critic（39,434 参数）；256 个确定性合成留出场景中，平均奖励 3.500993（受约束随机策略 2.066525）、高威胁覆盖率 0.706510（基线 0.510352）、重攻击覆盖率 0.627474（基线 0.340495）；权重/数据/脚本 SHA256 与 HTTP、`algolib` 严格真实验收均已闭环 | 使用真实脱敏任务调度轨迹离线评估及重训；当前合成场景指标不得解释为实战调度效果，`marl_dynamic_router` 仍需单独补权重 |
| M14 | 可解释 AI 模型 | `edl_evidential_verifier`、规则证据链 | `COMPLETE` | 已训练并固化 290 参数 Dirichlet EDL 验证头；1000 条确定性合成留出集 accuracy 0.903、macro-F1 0.901410、Brier 0.066736、10-bin ECE 0.035839，均记录数据/脚本/权重 SHA256；服务逐条返回输入特征、类别概率与证据、Dirichlet alpha/strength、认知/偶然不确定性、阈值理由，并分流 verified/rejected/manual-review；缺权重拒绝启动，HTTP 与 `algolib` 严格真实验收通过 | 使用真实脱敏检测标注进行跨传感器校准和漂移评估；当前合成指标不得解释为实际检测可靠性，人工复核必须回看源图像 |
| M15 | 多模态融合模型 | `multimodal_mamba_fusion`；补充候选 `imagebind_multimodal_encoder` | `COMPLETE` | 已训练并固化 412,688 参数的 Mamba 风格嵌入融合模型；确定性合成留出集 mean cosine 0.771683、RMSE 0.042234、p05 cosine 0.648176，均优于归一化均值基线 0.679153、0.050066、0.491344；支持不等长输入的补齐/截断及按 `sensor_id` 显式关联，记录权重/数据/脚本 SHA256，缺权重拒绝启动，HTTP 与 `algolib` 严格真实验收通过 | 当前 COMPLETE 范围是已编码向量的融合，不包含原始图像/SAR/雷达/文本编码；仍需离线打包 ImageBind 等预训练编码器，并用真实脱敏多模态配对数据重训评估 |
...
| M20 | 图神经网络模型 | `graph_relation_reasoner`；补充资产 ST-GNN 轨迹模型包 | `COMPLETE` | 已训练并固化 16,673 参数的两层密集消息传递 GNN，用于多目标编组关系边分类；600 个独立确定性合成留出图上 accuracy 0.992088、F1 0.962788、ROC AUC 0.999212，优于原距离/运动规则基线 F1 0.912402、ROC AUC 0.936968；训练/留出数据、脚本、safetensors 权重 SHA256 与 HTTP、`algolib` 严格真实验收均已闭环，缺权重或哈希不符时拒绝启动且不再规则 fallback | 使用真实脱敏多目标关系标注重训，按目标类型、图密度和未见编组分层评估；当前合成指标不得解释为实际编组识别精度 |

## 当前可验证基线

- 算法包静态检查：35/35 通过；M01、M02、M03、M04（2 个包）、M05、M06、M07、M08、M12、M13、M14、M15、M16、M20 共 15 个算法卡已为 `validated`，其余 20 个算法卡仍处于 `draft`。
...
- ONNX Runtime：四个 ONNX 包均通过 Python ORT 与 C++ ORT 双重真实验收；
