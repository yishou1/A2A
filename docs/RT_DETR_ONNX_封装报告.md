...
**日期：** 2026-09-16（权重与 ONNX 同步刷新）  
**对象权重：** `models/checkpoints/battlefield_rtdetr_unit17_coco25_ap_kaggle_best.pt`  
（文件名保持不变；内容已替换为 **unit17_ap_finetune best**，97 类机场+飞机强化续训。别名同内容：`battlefield_rtdetr.pt`。旧权重备份：`*.pre_ap_finetune.pt`）  
