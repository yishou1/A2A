...
Agent 宕机、不可达、超时、断心跳
  ...
  -> 写入 unavailable_reason / unavailable_error_code / unavailable_workflow_id / unavailable_work_item / unavailable_at
  -> 清理 lease_* 字段
```
...
```text
commander_agent/error_classification.py
a2a_protocol/client.py
a2a_protocol/messages.py
a2a_protocol/server.py
```

这次把原来分散的字符串判断收敛成统一错误分类。Commander 不再只靠一段错误文本猜测是否要切换，而是先把异常分类成明确的 `error_code`：

```text
AGENT_UNAVAILABLE      连接失败、连接重置等实例不可达错误
AGENT_TIMEOUT          请求超时
AGENT_NOT_READY        Agent ready=false 或暂不可用
AGENT_HEARTBEAT_LOST   运行中租约心跳丢失
AGENT_HTTP_5XX         Agent 侧 5xx 错误
AGENT_PROTOCOL_ERROR   认证、接口、协议类错误
AGENT_BUSINESS_ERROR   业务执行失败，例如参数非法、坐标错误
AGENT_LATE_RESPONSE    failover 后旧 Agent 迟到返回
```

其中只有系统不可用类错误会触发 failover：
...
```text
例如 Agent 返回 invalid coordinates，
Commander 会认为这是任务或参数问题，
不会盲目切换到备用 Agent 掩盖业务错误。
```

trace 和 Nacos metadata 中会记录：
...
```bash
conda run -n Agent python -m pytest tests/test_error_classification.py -q
```
