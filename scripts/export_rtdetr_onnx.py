...
# 默认与三档 profile 一致：文件名仍为 unit17_coco25_ap_kaggle_best；
# 当前内容为 unit17_ap_finetune best（机场+飞机续训）。缺省回退 battlefield_rtdetr.pt。
DEFAULT_CHECKPOINTS = (
    REPO_ROOT / "models" / "checkpoints" / "battlefield_rtdetr_unit17_coco25_ap_kaggle_best.pt",
    REPO_ROOT / "models" / "checkpoints" / "battlefield_rtdetr.pt",
)
...
def _resolve_checkpoint(explicit: str | None = None) -> Path:
    if explicit:
        path = Path(explicit)
    ...
    raise FileNotFoundError(
        "No RT-DETR checkpoint found. Tried:\n  - "
        + "\n  - ".join(str(p) for p in DEFAULT_CHECKPOINTS)
    )
...
agent_card:
  summary: >
    ...
    (97-class unit17 taxonomy; weights file keeps the coco25_ap_kaggle_best
    name, content is unit17_ap_finetune best for airport+plane). Caller
    supplies a letterboxed float32 NCHW image tensor; decode / letterbox /
    score filter / NMS / ODConv refine remain in the python_http_service sibling.
  when_to_use:
...
""",
    ...
    _write_json(
        ...
        {
            "model_source": str(ckpt.relative_to(REPO_ROOT)).replace("\\", "/"),
            ...
            "num_classes": nc,
            "parameter_count": param_count,
            "weights_lineage": "unit17_ap_finetune_best (filename kept as unit17_coco25_ap_kaggle_best)",
            "input": in0,
            "outputs": output_meta,
            "sibling_python_service": "battlefield_rtdetr_detector",
            "sibling_algorithms_package": "algorithms/battlefield_rtdetr_detector",
...
| Item | Value |
|------|-------|
| Weights source | `{str(ckpt.relative_to(REPO_ROOT)).replace(chr(92), '/')}` |
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--weights",
        default=None,
        help="Ultralytics .pt path (default: unit17_coco25_ap_kaggle_best, else battlefield_rtdetr.pt)",
    )
    ...
    try:
        ckpt = _resolve_checkpoint(args.weights)
    except FileNotFoundError as exc:
        print(exc, file=sys.stderr)
        return 1

    out_onnx = MODELS_DIR / "battlefield_rtdetr.onnx"
    print(f"Exporting {ckpt} -> {out_onnx} (opset={ONNX_OPSET}, imgsz={IMGSZ}) ...")
    _export_ultralytics(ckpt, out_onnx, force=args.force)

    names = _names_from_ckpt(ckpt)
