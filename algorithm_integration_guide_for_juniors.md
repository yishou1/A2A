...
本轮建议把 M01-M18 全部分发下去，但每个人都先完成 2 个低风险算法包，跑通注册、激活、调用闭环；剩余算法作为第二批扩展。这样既覆盖 18 类，又不会因为一开始选题太大导致接入流程卡住。

### 分工总表

| 同学 | 方向 | 覆盖算法 | 数量 |
|---|---|---|---:|
| 同学 A | 决策、规划、协同 | M09、M11、M14、M15、M16 | 5 |
| 同学 B | 感知、变化检测、聚类评估 | M01、M02、M08、M10 | 4 |
| 同学 C | 表征、分类、跟踪、时序、图模型 | M03、M04、M05、M06、M07 | 5 |
| 同学 D | 大模型、RAG、解释、规则推理 | M12、M13、M17、M18 | 4 |
...
```text
第一批：M09 回归与评分、M11 多属性决策与排序
第二批：M14 智能体、M15 强化学习、M16 联邦学习
```
...
planning_agent_service
rl_policy_selector
federated_state_sync
```

目标：

```text
输入候选目标、任务特征、约束指标、候选方案或状态
输出评分、等级、排序结果、推荐方案、调度动作或协同状态
```
...
```text
第一批：M08 聚类与数据关联、M10 随机森林与集成学习
第二批：M01 实时目标检测、M02 差分检测与变化检测
```
...
```text
输入图像、视频帧、多源观测或多维特征
输出目标框、变化区域、聚类 ID、关联关系、态势评估结果
```

### 同学 C：表征、分类、跟踪、时序与图模型类

负责：

```text
第一批：M04 特征编码与分类、M06 时间序列预测
第二批：M03 多模态融合、M05 多目标跟踪与定位、M07 图神经网络
```
...
```text
输入目标特征、历史轨迹、观测序列、图结构或多模态数据
输出类别概率、统一表征、未来轨迹点、状态估计、关系推理结果
```
...
```text
第一批：M13 检索增强生成、M18 规则/知识图谱与神经符号推理
第二批：M12 大语言模型、M17 可解释 AI 与证据推理
```
...
```text
输入任务文本、规则、证据、知识库上下文
输出结构化解释、检索证据、规则命中、风险提示、证据链
```

第一批合计 8 个算法包，优先保证每个人至少 2 个算法包完整跑通；第二批再把剩余 10 个算法包逐步补齐。
