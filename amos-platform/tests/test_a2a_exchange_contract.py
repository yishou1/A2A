...
def test_friendly_platforms_match_integrated_mission_field_names() -> None:
    ...
    # The supplemental confirmation UAV is staged and is not part of the
    # initial operator-visible force package.
    ...
    escort = next(platform for platform in platforms if platform["platform_id"] == "ESCORT-01")
    reconnaissance_uav = next(platform for platform in platforms if platform["platform_id"] == "AEW-01")
    assert escort["metadata"]["fuel_pct"] is not None
    assert reconnaissance_uav["metadata"]["position"]["alt_ft"] == 18_000
...
def test_gateway_snapshot_preserves_platform_energy_and_measurement_geometry() -> None:
    ...
    escort = next(asset for asset in snapshot["assets"] if asset["asset_id"] == "ESCORT-01")
    elevated_measurements = [
        item for item in snapshot["observations"]
        if item.get("elevation_deg") not in (None, 0, 0.0)
    ]

    assert escort["fuel_pct"] is not None
    assert "battery_pct" in escort
    assert elevated_measurements
...
def test_exchange_media_references_follow_the_causal_story_cutoff() -> None:
    ...
    assert "MAR-MEDIA-02" not in actual_ids
    assert sum(
        1 for event in engine.exchange.events_after(0)
        for item in event["media_refs"] if item["media_id"] == "MAR-MEDIA-00"
    ) == 1
...
def test_stage_transfer_scopes_media_and_declares_required_inputs() -> None:
    ...
    assert stage["checkpoint_id"] == "MAR-CP-PERCEPTION"
    ...
    assert "MAR-MEDIA-03" not in json.dumps(payload, ensure_ascii=False)
...
def test_stage_context_is_gateway_safe_and_event_media_refs_are_strict_deltas() -> None:
    ...
    engine.clock.update({
        ...
        "scenario_id": SCENARIO_ID,
        "scenario_branch": "standard",
        "director_checkpoint_id": "MAR-CP-ASSESS",
    })
    for _ in range(97):
        engine._tick(30)
    ...
    assert stage["incremental_media_ids"] == ["MAR-MEDIA-03", "MAR-MEDIA-04"]
    assert [item["id"] for item in payload["attachments"]] == stage["incremental_media_ids"]
    ...
    assert len([item for item in refs if item["media_id"] == "MAR-MEDIA-00"]) == 1
    ...
    electronic = next(
        ...
        if item["media_id"] == "MAR-MEDIA-03"
    )
    assert electronic["structured_data"]["schema_version"] == "amos.evidence-data.v1"
    assert electronic["structured_data"]["capture_geometry"]
    assert electronic["consumer_context"] == {
        "planned": True,
        "functional_role_ids": ["A2"],
        "model_requirement_ids": ["M05", "M16", "M19", "M20"],
        "execution_evidence": "backend_trace_required",
    }


def test_assessment_stage_withholds_post_action_frame_before_execution() -> None:
    scenario = get_scenario(SCENARIO_ID)
    engine = SimEngine(seed=int(scenario["default_seed"]))
    engine.load_scenario(scenario)
    engine.clock.update({
        ...
        "scenario_id": SCENARIO_ID,
        "scenario_branch": "standard",
        "director_checkpoint_id": "MAR-CP-CLOSE",
    })
    for _ in range(187):
        engine._tick(30)
    ...
    assert all(
        item["media_id"] != "MAR-MEDIA-07"
        for item in stage["evidence_items"]
    )
