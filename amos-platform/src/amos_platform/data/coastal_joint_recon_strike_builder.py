...
def _timeline() -> list[dict[str, Any]]:
    return [
        ...
        {"cue_id": "CJR-CUE-10", "at_sec": 4800, "phase": "ASSESS", "level": "INFO", "title": "任务资源返航与保障舰回收", "description": "海上联合指挥舰离开发射阵位，无侦-10完成复查后返航；歼-16沿空中返航线撤离，两架攻击无人机沿南北回收航线返回保障舰甲板。", "media_ids": [], "functional_agent_ids": ["A3", "A6"], "model_requirement_ids": ["M14"], "function_ids": ["KC-28"]},
    ]
...
def build_coastal_joint_recon_strike_scenario() -> dict[str, Any]:
    ...
    return {
        ...
        "id": SCENARIO_ID,
        "name": "临海多域协同侦察与精确打击",
        "operator_brief": "1艘海上联合指挥舰进入巴士海峡虚构任务海域后，低轨侦察卫星01在首个访问窗口形成待识别离岛接触，侦察卫星02在后续窗口接力复访，通信中继卫星持续保障星—舰链路；已经下传的情报产品在有效期内继续参与融合，不因卫星离开地图而消失。1架无侦-10按SAR、光电和电子侦察顺序复核；1架歼-16承担空中战术指挥和防区外打击；攻击无人机01、02沿北、南两条独立航路抵达释放阵位。后端完成识别与保护区审查、操作员授权后，四个火力节点按同一到达时刻实施模拟攻击，随后歼-16分路撤离，两架攻击无人机返回保障舰甲板，由无侦-10单机完成毁伤复查。",
        ...
        "engagement_policy": {
            ...
            "post_launch_routes": {
                ...
                "ATTACK-UAV-01": [
                    ...
                    {"lat": 20.18, "lng": 122.30, "label": "UAV01-RECOVERY-APPROACH"},
                    {"lat": 20.19, "lng": 122.28, "label": "SEA-C2-RECOVERY-DECK"},
                ],
                "ATTACK-UAV-02": [
                    {"lat": 20.10, "lng": 122.15, "label": "UAV02-EGRESS"},
                    {"lat": 20.16, "lng": 122.26, "label": "UAV02-RECOVERY-APPROACH"},
                    {"lat": 20.19, "lng": 122.28, "label": "SEA-C2-RECOVERY-DECK"},
                ],
            },
        ...
        "agent_plan": {
            ...
            "commander_options": {
                ...
                "knowledge_base": [
                    ...
                    "发射后歼-16立即执行空中脱离航线，攻击无人机01和02沿各自回收航线返回海上联合指挥与无人机保障舰；毁伤评估完成后无侦-10与指挥舰分别返航和撤离。",
                ],
            },
        },
